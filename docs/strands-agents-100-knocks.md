...
## 第11章 ループエンジニアリング(101〜107)【発展・横断テーマ】

> 対応ドキュメント: [loop-engineering.md](handson/loop-engineering.md)(100本全体をループ制御の観点で再整理)
>
> エージェント=「観察→行動→観察」の外側ループ。その**ループ自体を設計対象**にする発展章。
> 第1〜10章の部品を「ループのどこを制御するか」で束ね直す。全ノック AWS 不要(MockModel)。

| No | タイトル | 学ぶこと | 難易度 |
|---|---|---|---|
| 101 | ループの解剖 | `stop_reason` の全分類で「なぜ止まったか」を区別する | ★★ |
| 102 | 反復回数で縛る | `Limits(turns=)` で有界化し、続きから再開する | ★★ |
| 103 | トークン予算で縛る | `Limits(total_tokens=)` でコストを上限で止める | ★★ |
| 104 | 自己修正ループ | Reflexion(生成→自己批評→改稿)を合格/上限まで | ★★★ |
| 105 | 検証ループ | loop-until-verified(外部チェッカを通るまで有界に再試行) | ★★★ |
| 106 | 予算駆動ループ | 予算が尽きるまで探索を深める(深さを予算で決める) | ★★★ |
| 107 | 暴走ガード | 同一アクションの反復を検出して止める(内側の安全網) | ★★★ |

**ループ設計の3原則**: ①有界であれ(102・103・107)②各周を意味あるものに(104・105・106)
③止まり方を設計せよ(101)。詳細は [handson/chapter11.md](handson/chapter11.md)。
